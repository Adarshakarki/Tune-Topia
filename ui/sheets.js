...
export function openTrackSheet(track, opts = {}) {
  ...
  if (likeIcon) likeIcon.className = `bi ${liked ? 'bi-heart-fill' : 'bi-heart'}`;
...
// ── SLEEP ─────────────────────────────────────────────────
function _openSleepSheet()  { $('sleep-timer-popup')?.classList.add('open'); UI.closeMoreSheet(); }
function _closeSleepSheet() { $('sleep-timer-popup')?.classList.remove('open'); }
...
function _cancelSleepTimer() {
  ...
  const status = $('sleep-timer-status'), cancel = $('sleep-timer-cancel-btn');
...
function _setSleepTimer(mins) {
  ...
  const hhmm    = endTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
  const status  = $('sleep-timer-status'), cancel = $('sleep-timer-cancel-btn');
  if (status) { status.textContent = `Pausing at ${hhmm} (${mins} min)`; status.style.display = ''; }
...
export function initEvents() {
  ...
  $('tsheet-go-artist')?.addEventListener('click', () => {
    ...
    import('../pages/artist.js').then(m => {
      ...
      if (track.artistId) {
        ...
        m.open({ id: '', name: track.artist?.split(',')[0].trim(), cover: track.cover });
      }
  ...
  $('tsheet-go-album')?.addEventListener('click', () => {
    ...
    const track = _tsTrack;
    _closeTrackSheet();
    import('../pages/album.js').then(m => {
      if (!track.albumId) { UI.toast('Album info not available for this track'); return; }
      const page = document.getElementById('page-album');
      if (page) page.classList.add('stacked');
      m.open({ id: track.albumId, title: track.album, cover: track.cover, artist: track.artist });
  ...
  $('tsheet-download')?.addEventListener('click', async () => {
    ...
    try {
      ...
      const stream = track.source === 'youtube' ? await getAudioStream(track.id) : await getStream(track.id);
      const url    = stream.url;
      if (!url) throw new Error('No direct URL (DASH streams cannot be downloaded yet)');
  ...
  $('tsheet-move-up')?.addEventListener('click',   () => { _tsOpts.onMoveUp?.();   _closeTrackSheet(); });
  $('tsheet-move-down')?.addEventListener('click', () => { _tsOpts.onMoveDown?.(); _closeTrackSheet(); });
  $('tsheet-remove')?.addEventListener('click',    () => { _tsOpts.onRemove?.();   _closeTrackSheet(); });
  ...
  $('sleep-timer-cancel-btn')?.addEventListener('click', () => { _cancelSleepTimer(); _closeSleepSheet(); });
  ...
  document.querySelectorAll('.sleep-timer-opt').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (btn.dataset.mins === 'eot') {
        ...
        const status = $('sleep-timer-status'), cancel = $('sleep-timer-cancel-btn');
        if (status) { status.textContent = 'Pausing after current track'; status.style.display = ''; }
        if (cancel) cancel.style.display = '';
  ...
  $('sheet-add-queue')?.addEventListener('click', () => {
    ...
    if (t) { Queue.addNext(t); UI.toast('Added to queue'); UI.closeMoreSheet(); }
  });
  ...
  $('sheet-open-source')?.addEventListener('click', () => {
    const t = Player.getCurrentTrack(); if (!t) return;
    window.open(t.source === 'youtube' ? `https://youtu.be/${t.id}` : `https://tidal.com/track/${t.id}`, '_blank');
    UI.closeMoreSheet();
  });
  ...
  $('np-queue-list')?.addEventListener('click', (e) => {
    ...
    $('qsheet-move-down').style.display = index >= tracks.length - 1 ? 'none' : '';
  ...
  $('qsheet-play-now')?.addEventListener('click',  () => { if (_queueSheetIndex < 0) return; Player.playFromQueue(_queueSheetIndex); _closeQueueSheet(); });
  $('qsheet-move-up')?.addEventListener('click', () => {
    if (_queueSheetIndex < 0) return;
    const pos = State.get('player.queuePosition') || 0;
    if (_queueSheetIndex <= pos + 1) { UI.toast('Already at the top'); _closeQueueSheet(); return; }
    Queue.reorder(_queueSheetIndex, _queueSheetIndex - 1);
    _closeQueueSheet(); _refreshQueue();
  });
  $('qsheet-move-down')?.addEventListener('click', () => {
    if (_queueSheetIndex < 0) return;
    const tracks = State.get('queue.tracks') || [];
    if (_queueSheetIndex >= tracks.length - 1) { UI.toast('Already at the bottom'); _closeQueueSheet(); return; }
    Queue.reorder(_queueSheetIndex, _queueSheetIndex + 1);
    _closeQueueSheet(); _refreshQueue();
  });
  ...
  $('np-queue-clear-btn')?.addEventListener('click', () => { Queue.clear(); _refreshQueue(); UI.toast('Queue cleared'); });
}