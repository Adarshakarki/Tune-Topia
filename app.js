...
const App = (() => {
  ...
  function initSearch() {
    ...
    document.querySelectorAll('.ios-cat-tile').forEach(tile => {
      tile.addEventListener('click', () => {
        ...
        lastQuery = tile.textContent.trim();
        $('search-results').innerHTML = UI.skeletons();
        showSearchResults();
        runSearch(lastQuery, 'music');
      });
    });
  }

  function showSearchEmpty() {
    const empty = $('search-empty-state');
  ...
  async function runSearch(q, tab) {
    ...
    try {
      if (tab === 'music') {
        let tracks = [];
        try { tracks = await hifiSearch(q); } catch {}
        if (!tracks.length) tracks = await ivSearch(q);
        UI.renderTracks(tracks, el, currentId);
      } else if (tab === 'video') {
        const tracks = await ivSearch(q);
        UI.renderTracks(tracks, el, currentId);
      } else if (tab === 'albums') {
        const albums = await hifiSearchAlbums(q);
        UI.renderAlbums(albums, el);
      } else if (tab === 'artists') {
        const artists = await hifiSearchArtist(q);
  ...
  function initPlayerControls() {
    ...
    $('np-content').addEventListener('touchmove', e => {
      if (e.touches[0].clientY - ty0 > 80) UI.closePlayer();
    }, { passive: true });
  }
  ...
  function init() {
    ...
    // library menu items
    $('lib-recently-added')?.addEventListener('click', () => { showPage('library'); });
...
const DesktopApp = (() => {
  ...
  async function loadDtNew() {
    ...
    grid.innerHTML = dtSkeletonGrid();
    try {
      const tracks = await search('new music releases 2025');
      renderDtAlbumGrid(tracks, grid, 'tracks');
    } catch (e) { grid.innerHTML = dtEmpty('Failed to load', e.message); }
  }
  ...
  async function loadDtAlbums() {
    ...
    grid.innerHTML = dtSkeletonGrid();
    try {
      const albums = await hifiSearchAlbums('popular albums 2025');
      renderDtAlbumGrid(albums, grid, 'albums');
    } catch {
      try { const t = await search('popular albums 2025'); renderDtAlbumGrid(t, grid, 'tracks'); }
  ...
  async function loadDtSongs() {
    ...
    el.innerHTML = Array(12).fill(0).map(() => `<div class="dt-skel-row skeleton" style="margin-bottom:2px"></div>`).join('');
    try {
      let t = [];
      try { t = await hifiSearch('popular songs 2025'); } catch {}
  ...
  async function loadDtArtists() {
    ...
    try {
      const a = await hifiSearchArtist('popular artists 2025');
      renderDtArtistGrid(a, grid);
    } catch (e) { grid.innerHTML = dtEmpty('Failed', e.message); }
  }

  async function loadDtRecentlyAdded() {
    const grid = document.getElementById('dt-recent-grid');
    const hist = Player.loadHistory();
    if (!hist.length) { grid.innerHTML = dtEmpty('Nothing here yet', 'Play some tracks to see them here'); return; }
  ...
  function initDtSearch() {
    ...
    input.addEventListener('input', () => {
      ...
      document.getElementById('dt-search-results').innerHTML = Array(8).fill(0).map(() => `<div class="dt-skel-row skeleton" style="margin-bottom:2px;border-radius:6px"></div>`).join('');
  ...
  async function dtSearchQuery(q, tab = 'music') {
    ...
    if (!el) return;
    el.innerHTML = (tab === 'albums') ? dtSkeletonGrid() : Array(10).fill(0).map(() => `<div class="dt-skel-row skeleton" style="margin-bottom:2px;border-radius:6px"></div>`).join('');
    try {
      ...
      else if (tab==='albums'){ const a=await hifiSearchAlbums(q); el.className='dt-album-grid'; renderDtAlbumGrid(a, el, 'albums'); }
  ...
  function dtSkeletonGrid(n = 10, shape = 'square') {
    return `<div class="dt-skel-grid">${Array(n).fill(0).map(() => `
      <div><div class="dt-skel-art skeleton" style="${shape==='circle'?'border-radius:50%':''}"></div><div class="dt-skel-line skeleton" style="width:85%"></div><div class="dt-skel-line-sm skeleton"></div></div>
    `).join('')}</div>`;
  }

  function dtEmpty(msg, sub = '') {
    return `<div class="dt-empty"><svg width="56" height="56" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.2"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/></svg><h3>${escHtml(msg)}</h3>${sub ? `<p>${escHtml(sub)}</p>` : ''}</div>`;
  }
